...
export default class ContractInstance {
  ...
  getApplicableActions( pubkeys ){
    if (pubkeys === undefined)
      pubkeys = [this._context.principalIdentity.getPublicKey()];
    return this._mcc.messageChain.getContractState().getApplicableActions( pubkeys );
  } 

  getActionParams(actionName){
     return this._mcc.messageChain.getContractState().getActionParams(actionName);
  }
