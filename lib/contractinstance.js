...
export default class ContractInstance {
  ...
  getApplicableActions( pubkeys ){
    ...
    getAllApplicableActions() {
        ...
        const allPubkeys = [];
        for (const fn of Object.keys(fieldInfo)) {
            // TODO: handle arrays
            if (fieldInfo[fn].type.name === "PUBKEY") {
                if (fields[fn]) {
                    allPubkeys.push(Buffer.from(fields[fn], 'hex'));
                }
            }
        }
        return this.getApplicableActions(allPubkeys);
    }
